 ...
 * Things to implement, smooth turning, smooth out deadlock, always forwards, 
 */
...
void cnn_guided_periodic(void)
{
  ...
  float speed = fminf(oag_max_speed,forward_conf);
  float moveDistance = fminf(maxDistance, 0.2f * forward_conf);
  float turningrate = fminf(max_heading_rate, oag_heading_rate*(right_conf-left_conf));
  ...
  switch (navigation_state){
    case PATH_FOLLOWING:
      ...
      if (!InsideObstacleZone(WaypointX(WP_TRAJECTORY),WaypointY(WP_TRAJECTORY))){
        ...
        guidance_h_set_heading_rate(turningrate);
